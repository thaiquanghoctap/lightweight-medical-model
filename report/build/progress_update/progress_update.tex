...
XAI bằng Grad-CAM & Đã có script & Đã bổ sung \texttt{generate\_gradcam.py}; cần chạy trong môi trường có PyTorch để sinh ảnh heatmap đưa vào report. \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Classification & \path{colab-result/lightweight-medical-model/outputs/busi/classification_cu/img_224/cbam/run_1/best_model.pt} \\
Segmentation & \path{colab-result/lightweight-medical-model/outputs/busi/segmentation/img_224/cbam/best_model.pt} \\
MedNetMultiTask & \path{colab-result/lightweight-medical-model/outputs/busi/multi/img_224/cbam/seg_weight_1/best_model.pt} \\
R-CBAM MNet & \path{colab-result/lightweight-medical-model/outputs/busi/r_cbam_mnet/img_224/seg_weight_1.2/lr_0.001/weight_decay_0.001/patience_100/}; thư mục này lưu \texttt{best_classification.pt}, \texttt{best_segmentation.pt} và \texttt{best_joint.pt}, không phải \texttt{best_model.pt}. \\
MK-MNet width 0.25 & \path{colab-result/lightweight-medical-model/outputs/busi/mk_mnet/img_224/width_0.25/lambda_0.8/lr_0.0003/weight_decay_0.0001/patience_100/best_model.pt} \\
MK-MNet width 0.5 & \path{colab-result/lightweight-medical-model/outputs/busi/mk_mnet/img_224/width_0.5/lambda_0.8/lr_0.003/weight_decay_0.001/patience_100/best_model.pt} \\
Logs và metrics & Các thư mục checkpoint trên có \path{result.csv} và/hoặc \path{epoch_log.csv} tương ứng. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=\textbf{G\arabic*.}]
    ...
    \item \textbf{Bổ sung XAI và phân tích trực quan}. Nhóm đã bổ sung script \texttt{generate\_gradcam.py} cho nhánh classification, dùng feature map ở stage cuối của backbone để sinh Grad-CAM. Bước còn lại là chạy script trên checkpoint đại diện, lưu ảnh heatmap vào \path{report/report_images/} và phân tích các mẫu đúng/sai theo từng lớp bệnh. Nếu Grad-CAM lệch khỏi mask tổn thương, nhóm sẽ phân tích khả năng mô hình đang học shortcut từ texture, nhiễu hoặc artifact trong ảnh siêu âm.
...
Mốc 4 & Phân tích sâu, cải thiện model và bổ sung XAI. & Đang hoàn thiện: đã có script Grad-CAM, còn cần sinh ảnh phân tích, cải thiện segmentation loss và thêm metric y khoa. \\
