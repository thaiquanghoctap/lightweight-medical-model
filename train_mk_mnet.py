...
def run_epoch(
    ...
    with torch.set_grad_enabled(is_training):
        for images, labels, masks in tqdm(loader, desc=description, leave=False):
            ...
            if len(outputs) == 3:
                classification_logits, segmentation_logits, auxiliary_logits = outputs
            else:
                classification_logits, segmentation_logits = outputs
                auxiliary_logits = []
            ...
            for aux_logits in auxiliary_logits:
                total_segmentation_loss = total_segmentation_loss + aux_weight * segmentation_criterion(
                    aux_logits, masks
                )
            loss = classification_loss + segmentation_weight * total_segmentation_loss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MK-MNet on BUSI.")
